...
const LeagueMatches = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>('round_robin');
  ...
  const fetchLeague = async () => {
    ...
    if (!error && data) {
      setLeague(data);
      if (data.league_type === 'group_stage_knockouts') {
        setActiveTab('group_1');
      }
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {league?.league_type === 'group_stage_knockouts' ? (
              <>
                {Array.from({ length: league.num_groups || 2 }, (_, i) => i + 1).map(g => (
                  <button
                    key={g}
                    onClick={() => setActiveTab(`group_${g}`)}
          ...
          <div className="space-y-3">
            {(() => {
              ...
              if (activeTab === 'knockouts') {
                displayMatches = knockoutMatches;
              } else if (activeTab.startsWith('group_')) {
                const groupNum = parseInt(activeTab.split('_')[1]);
                displayMatches = roundRobinMatches.filter(m => m.group_number === groupNum);
              } else {
                displayMatches = roundRobinMatches;
              }
                ...
                return (
                  <div className="card text-center py-12">
                    ...
                    <p className="text-gray-400">
                      Matches will appear here once generated by the admin.
                    </p>
                  </div>
                );
              }
